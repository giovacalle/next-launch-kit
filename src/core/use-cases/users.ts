...
export async function createUserWithCredentialsUseCase(
  ...
  const token = await upsertEmailVerificationToken(newUser.id);
...
export async function createUserWithMagicLinkUseCase(email: string) {
  ...
  const newUser = await createUserWithMagicLink(email);
  if (!newUser) throw new Error('Something went wrong');

  // info: maybe we should create a user profile with random autogenerated data
  await createUserProfile(newUser.id, 'Guest', 'Magic link');

  const token = await upsertMagicLink(newUser.id);
